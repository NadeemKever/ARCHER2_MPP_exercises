...
int main(int argc, char* argv[]){
    ...
    if (M%P!= 0 ){
        if (rank ==0) cout<< "ERROR: Code requires number of procs to be divide "<< M<< " evenly"<< endl;
        MPI_Finalize();
        return 0;
    ...
    if (rank ==0){
        printf("\n\n\nWriting on rank %d\n",rank);
        string newFilename = "ppicture" + to_string(atoi(argv[1])) + ".pgm";
